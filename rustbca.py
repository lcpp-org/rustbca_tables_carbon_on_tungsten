...
def sputtering(ions, target, energies, angle, N_=10000, run_sim=True):
    ...
    for option_index, option in enumerate(options):
        ...
        for index, energy in enumerate(energies):
            ...
            if run_sim: os.system(f'../target/release/RustBCA {name+str(index)+"_"+str(option_index)}.toml')
...
def main():
    ...
    target = tungsten
    energies = np.logspace(np.log10(30), 3, 100)
    ...
    for ion in ions:
        ...
        for ie in range(num_energies):
            ...
            for ia in range(num_angles):
                ...
                # Sputtering yield
                if np.shape(s)[1] > 0:
                    SY = len(s)/number_ions
                else:
                    SY = 0.0
                sputtering_yield[ie][ia] = SY
                # Reflection coefficient
                if np.shape(r)[1] > 0:
                    RC = len(r)/number_ions
                else:
                    RC = 0.0
